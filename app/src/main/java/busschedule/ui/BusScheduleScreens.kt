...
import busschedule.ui.theme.BusScheduleTheme
...
@Composable
fun BusScheduleApp(
    viewModel: BusScheduleViewModel = viewModel(factory = BusScheduleViewModel.Companion.factory)
) {
    val navController = rememberNavController()
    val fullScheduleTitle = stringResource(R.string.full_schedule)
    var topAppBarTitle = remember { mutableStateOf(fullScheduleTitle) }
    val fullSchedule = viewModel.getFullSchedule().collectAsState(emptyList())
    val onBackHandler = {
        topAppBarTitle.value = fullScheduleTitle
